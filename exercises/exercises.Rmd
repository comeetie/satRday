...
Import the restaurant layer 'sir.shp' and display a map of Paris with its restaurants simply with `plot` function. 
```

```{block, opts.label = "clues"}
Use `sf::st_read()` and `sf::st_geometry()`. You can also customize many options of `plot` function : bg, col, lwd, border, pch, cex...
```
...
Using the layer called ‘iris_75’, create a new aggregated map layer called ‘com_75’ which corresponds to ‘arrondissements’ (districts) of Paris . Also keep in this new layer the information on the population, area and number of restaurant in each district.
```
...
map_ggplot <- ggplot() +
  ...
  geom_sf(data = com_75 %>%  st_centroid(),
          aes(size= RESTAU),colour="#f5f5f5", alpha=0.8,show.legend = "point") +
  ...
  theme_minimal() +
  theme(panel.background = element_rect(fill = "cornsilk",color=NA),legend.position = "bottom", plot.background = element_rect(fill = "cornsilk",color=NA))+
  labs(
    title = "Restaurants Distribution in Paris",
    caption = "Insee & SIRENE, 2018\nKim, Tim & Comeetie, 2019"
  )+guides( size = guide_legend(label.position = "bottom", title.position = "top",override.aes = list(alpha = 1,color="#ffffff")),
...
In this exercice we will use `mapview` and `leaflet` to explore two data sources on restaurants: 

* Siren which we already used previously 
...
* `osmdata:opq()` to define the boudning box of the osm request
...
* computes the centroids for the restaurants stored as polygons or multi-polygons 
* cleans the points by removing the ones that were extracted from exploding the polygons.
